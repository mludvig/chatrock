...
- **`.composer-toolbar`** (in `ChatView.tsx`, above the textarea) — decisions about the message you're about to send: model, thinking effort (when supported), research depth, project picker (drafts only — a native `<select>` that collapses to a folder icon via `.project-picker-wrap.is-empty` while nothing is filed, and spends width on the ellipsised name only once a project is chosen), and Private quick-toggle. Reasoning controls are session-scoped overrides seeded from saved defaults; see `docs/adr/0043-composer-owns-reasoning-controls.md`. Scrolls horizontally rather than wrapping. Controls use `.composer-select` (pill-shaped); `.model-select` is the squarer variant still used inside dialogs and panels.

...
const researchEvent = (chatId = 'c1') => ({
  requestContext: { authorizer: { jwt: { claims: { sub: 'user-1' } } } },
  routeKey: 'GET /api/chats/{chatId}/research',
  pathParameters: { chatId },
  queryStringParameters: {},
})

test('GET /api/chats/{chatId}/research returns 404 if the chat is not owned', async () => {
  mockDynamo.getChat.mockResolvedValue(undefined)

  const res = result(await handler(researchEvent('other') as any))

  expect(res.statusCode).toBe(404)
  expect(mockDynamo.getActiveRun).not.toHaveBeenCalled()
})

test('GET /api/chats/{chatId}/research returns run: null when no run exists', async () => {
  mockDynamo.getChat.mockResolvedValue({ PK: 'USER#user-1', SK: 'CHAT#c1' })
  mockDynamo.getActiveRun.mockResolvedValue(undefined)
  mockDynamo.listRuns.mockResolvedValue([])

  const res = result(await handler(researchEvent() as any))

  expect(res.statusCode).toBe(200)
  expect(JSON.parse(res.body as string)).toEqual({ run: null })
})

test('GET /api/chats/{chatId}/research prefers the active run and shapes the DTO', async () => {
  mockDynamo.getChat.mockResolvedValue({ PK: 'USER#user-1', SK: 'CHAT#c1' })
  mockDynamo.getActiveRun.mockResolvedValue({
    runId: 'run-1',
    status: 'running',
    question: 'What is X?',
    plan: { subQuestions: [{ id: 'sq1', question: 'X?' }], clarifyingQuestions: [] },
    findings: [{ subQuestionId: 'sq1', summary: 'X is Y.', sourceUrls: [] }],
...
test('GET /api/chats/{chatId}/research falls back to the most recently created run when none is active', async () => {
  mockDynamo.getChat.mockResolvedValue({ PK: 'USER#user-1', SK: 'CHAT#c1' })
  mockDynamo.getActiveRun.mockResolvedValue(undefined)
  mockDynamo.listRuns.mockResolvedValue([
    { runId: 'run-old', status: 'done', question: 'Old?', createdAt: '2026-01-01T00:00:00Z', reportText: 'old report' },
    { runId: 'run-new', status: 'done', question: 'New?', createdAt: '2026-01-02T00:00:00Z', reportText: 'new report' },
  ])

  const res = result(await handler(researchEvent() as any))

  expect(JSON.parse(res.body as string).run.runId).toBe('run-new')
  expect(JSON.parse(res.body as string).run.reportText).toBe('new report')
})

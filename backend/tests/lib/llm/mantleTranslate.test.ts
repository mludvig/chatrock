...
describe('mantleTranslate: toNeutral (output items -> Block[])', () => {
  test('message item with output_text becomes a text block', () => {
    ...
    expect(toNeutral(items)).toEqual([{ kind: 'text', text: 'hello' }])
  })
  ...
  test('message item with a refusal part surfaces as text', () => {
    ...
    expect(toNeutral(items)).toEqual([{ kind: 'text', text: 'cannot help with that' }])
  })
  ...
  test('function_call item becomes a tool_call block, arguments JSON-parsed', () => {
    ...
    expect(toNeutral(items)).toEqual([{ kind: 'tool_call', callId: 'call_abc123', name: 'web_search', input: { query: 'weather' } }])
  })
  ...
  test('function_call item with malformed arguments JSON falls back to {}', () => {
    ...
    expect(toNeutral(items)).toEqual([{ kind: 'tool_call', callId: 'call_x', name: 'web_search', input: {} }])
  })
  ...
  test('unrecognized item types (e.g. web_search_call) are dropped', () => {
    ...
    expect(toNeutral(items)).toEqual([{ kind: 'text', text: 'ok' }])
  })
...
describe('mantleTranslate: fromNeutralMessages (NeutralMessage[] -> flat input items)', () => {
  ...
  test('assistant text message -> one message item with output_text (not input_text)', () => {
    ...
    const items = fromNeutralMessages(messages)
  ...
  test('tool_result with an image entry produces an array output with input_image', () => {
    ...
    const items = fromNeutralMessages(messages) as Array<{ type: string; output: unknown }>
  ...
  test('empty messages array -> empty items array', () => {
    expect(fromNeutralMessages([])).toEqual([])
  })
...
describe('mantleTranslate: round-trip (toNeutral then fromNeutralMessages)', () => {
  test('a full round-trips text + reasoning + tool_call through both directions', () => {
    ...
    const neutral = toNeutral(items)
    const roundTripped = fromNeutralMessages([{ role: 'assistant', content: neutral }])

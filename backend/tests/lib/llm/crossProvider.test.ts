...
// failure on Converse and meaningless on Responses) while leaving everything else
...
import { bedrockResponsesProvider } from '../../../src/lib/llm/providers/bedrockResponses'
...
function mixedProviderHistory(): NeutralMessage[] {
  ...
  const responsesOpaque = encodeOpaque('bedrock-responses', { id: 'rs_1', encryptedContent: 'GPT_ENC' })
  return [
    ...
    { role: 'assistant', content: [
      { kind: 'thinking', text: 'gpt reasoning', opaque: responsesOpaque },
...
describe('cross-provider sanitizeHistory symmetry', () => {
  test('Converse keeps its own thinking block, drops the Responses one, keeps text and tool call ids unrewritten', () => {
  ...
  test('Responses keeps its own thinking block, drops the Converse one, keeps text and tool call ids unrewritten', () => {
    const sanitized = bedrockResponsesProvider.sanitizeHistory(mixedProviderHistory())
    const allBlocks = sanitized.flatMap(m => m.content)

    const thinkingBlocks = allBlocks.filter(b => b.kind === 'thinking')
    expect(thinkingBlocks).toHaveLength(1)
    expect(thinkingBlocks[0]).toMatchObject({ text: 'gpt reasoning' })
    expect((thinkingBlocks[0] as Extract<typeof thinkingBlocks[0], { kind: 'thinking' }>).opaque?.provider).toBe('bedrock-responses')
  ...
  test('a history with only foreign opaque (no same-provider material at all) sanitizes without throwing, for both providers', () => {
    const onlyResponses: NeutralMessage[] = [
      { role: 'assistant', content: [{ kind: 'thinking', text: 'gpt', opaque: encodeOpaque('bedrock-responses', { id: 'rs_1' }) }] },
      { role: 'assistant', content: [{ kind: 'text', text: 'answer' }] },
    ]
    expect(() => bedrockConverseProvider.sanitizeHistory(onlyResponses)).not.toThrow()
    expect(bedrockConverseProvider.sanitizeHistory(onlyResponses).flatMap(m => m.content).some(b => b.kind === 'thinking')).toBe(false)
    ...
    expect(() => bedrockResponsesProvider.sanitizeHistory(onlyConverse)).not.toThrow()
    expect(bedrockResponsesProvider.sanitizeHistory(onlyConverse).flatMap(m => m.content).some(b => b.kind === 'thinking')).toBe(false)
  })
})

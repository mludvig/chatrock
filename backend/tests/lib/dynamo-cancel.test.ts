...
import { ddb, setStreamCancel, isStreamCancelled, clearStreamCancel, buildChatKey } from '../../src/lib/dynamo'
...
test('setStreamCancel issues UpdateCommand setting cancelRequested=true on the chat key', async () => {
  mockSend.mockResolvedValue({})
  await setStreamCancel('user-1', 'chat-abc')
...
test('isStreamCancelled returns true when cancelRequested=true in the item', async () => {
  mockSend.mockResolvedValue({ Item: { ...buildChatKey('user-1', 'chat-x'), cancelRequested: true } })
  const result = await isStreamCancelled('user-1', 'chat-x')
  expect(result).toBe(true)
  const cmd = mockSend.mock.calls[0][0]
  expect(cmd).toBeInstanceOf(GetCommand)
  expect(cmd.input.Key).toEqual(buildChatKey('user-1', 'chat-x'))
})

test('isStreamCancelled returns false when cancelRequested is absent', async () => {
  mockSend.mockResolvedValue({ Item: { ...buildChatKey('user-1', 'chat-y') } })
  expect(await isStreamCancelled('user-1', 'chat-y')).toBe(false)
})

test('isStreamCancelled returns false when item does not exist', async () => {
  mockSend.mockResolvedValue({})
  expect(await isStreamCancelled('user-1', 'chat-gone')).toBe(false)
})

test('clearStreamCancel issues UpdateCommand removing cancelRequested', async () => {
  mockSend.mockResolvedValue({})
  await clearStreamCancel('user-1', 'chat-abc')
  expect(mockSend).toHaveBeenCalledTimes(1)
  const cmd = mockSend.mock.calls[0][0]
  expect(cmd).toBeInstanceOf(UpdateCommand)
  expect(cmd.input.Key).toEqual(buildChatKey('user-1', 'chat-abc'))

...
- `enrichUserFacts`/`enrichProjectFacts`/`summarizeChat`/title generation (`ws/sendMessage.ts`'s post-turn enrichment block) are skipped entirely when `chat.isPrivate` — gated by the same `if` as `memoryEnabled`, so a private chat's title just stays `"New Chat"`. This is the part most likely to leak private content if missed.
- The manual trigger equivalents — `POST /api/chats/{chatId}/retitle` and `POST /api/chats/{chatId}/resummarize` (`http/chats.ts`) — also reject with 400 for a private chat, even though neither is reachable from the UI for one today (both buttons live in list/project views that never show private chats). Guarding at the route, not just the UI, matters because it's the only thing stopping a direct API call from running private content through `TITLE_MODEL`/`MEMORY_EXTRACTION_MODEL` anyway.

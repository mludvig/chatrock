...
test('project defaults survive reload and can be reset; project creation stays a draft', async ({ page }) => {
  ...
  try {
    ...
    page.on('request', r => { if (r.method() === 'POST' && r.url().endsWith('/api/chats')) creations.push(r.url()) })
...
test('project home carries a question into a draft without sending it', async ({ page }) => {
  ...
  try {
    ...
    await page.locator('.message-input').fill('An edited project question')
    await page.locator('.message-input').blur()
    await expect.poll(() => page.evaluate(() => Object.values(localStorage).some(value => value.includes('An edited project question')))).toBe(true)
    await page.reload()
    await expect(page.locator('.message-input')).toHaveValue('An edited project question')
...
test('project facts can be added and edited and files can be opened', async ({ page }) => {
  ...
  try {
    ...
    await page.getByLabel('Fact actions', { exact: true }).click()
    await page.getByRole('button', { name: 'Edit fact', exact: true }).click()
    await page.getByLabel('Edit text').fill('Use metric units, including millimetres for dimensions.')
    ...
    const url = await page.getByRole('link', { name: 'Open / download file' }).getAttribute('href')
    ...
    await page.reload()
    await page.getByRole('button', { name: /Knowledge/ }).click()
    await expect(page.getByText('Use metric units, including millimetres for dimensions.', { exact: true })).toBeVisible()
  } finally { await request(page, 'DELETE', `/projects/${p.projectId}`) }
})
...
test('recent chats remain in the visible sidebar and effort is a direct control', async ({ page }) => {
  ...
  try {
    ...
    await expect(page.getByRole('button', { name: 'Show private chats', exact: true })).toHaveAttribute('aria-pressed', 'false')

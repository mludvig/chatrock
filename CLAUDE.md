...
backend/src/
  ...
  lib/enrichment.ts       — post-turn extraction: enrichUserFacts() (Sonnet) returns updated user memory list; enrichProjectFacts() (Sonnet) returns updated project memory list (no summary); summarizeChat() (Sonnet) merges the latest exchange into a running chat summary + topics[], independent of project membership; generateChatTitle() (Haiku) is a separate, independent call so a memory-JSON parse failure can never suppress titling; summarizeChatById() wraps summarizeChat() for immediate (non-per-turn) use, e.g. when a chat is moved into a project; all five log on failure instead of swallowing silently
...
- **Passive enrichment** (`sendMessage.ts` post-turn block, `lib/enrichment.ts`): runs per turn when `memoryEnabled`. **The `existing` lists it reconciles against are re-read fresh (`listUserMemories`/`listProjectMemories`) AFTER the agentic loop — NOT the pre-loop `userMemoriesRaw`/`projectMemoriesRaw` snapshots used for the system prompt.** This is load-bearing: the model may have written via `manage_memory`/`manage_project_memory` mid-loop, and passive enrichment must see those writes (with their memIds) to retain/merge them — otherwise it re-derives a paraphrase of the same fact as a NEW item (the dual-writer duplicate). The passive prompts also carry an explicit "some facts may already have been saved this turn (in the list with a memId, possibly worded differently) — merge and reuse the memId, don't restate" rule reinforcing this. `enrichUserFacts(transcript, existing, chatId?)` (Sonnet, JSON response) always runs and returns an updated user memory list; `reconcile()` + `putUserMemory` persist user ADDs/UPDATEs/DELETEs. `summarizeChat(transcript, existingSummary, existingTopics, chatId?)` (Sonnet, JSON response) also always runs — independent of project membership — merging the latest exchange into the chat's running summary + `topics[]`; persisted via `updateChatSummary(sub, chatId, {summary, topics})` when either is non-empty. When the chat belongs to a project, `enrichProjectFacts(transcript, existing, chatId?)` (Sonnet, JSON response) additionally returns an updated project memory list; `reconcile()` + `putProjectMemory` persist project ops. Title generation is a **separate, independent call** — `generateChatTitle(transcript, chatId?)` (Haiku, plain-text response, no JSON parsing) — gated by `needTitle` (first normal send while `chat.title === 'New Chat'`); on success: `updateChatTitle` + `titleUpdated` frame. Splitting title and summary out of the memory-extraction JSON means a malformed/truncated response in one call can never silently suppress another — each call has its own try/catch and logs on failure (`enrich_user_facts_error`/`_parse_error`, `summarize_chat_error`/`_parse_error`, `enrich_project_facts_error`/`_parse_error`, `generate_title_error`/`_empty_response`) rather than swallowing silently. Errors in the DB-write phase (outside these calls) emit one `warning` WS frame and log `enrich_turn_error`.
...
- **Chat summaries + topics**: every chat — not just project ones — gets `chat.summary` and `chat.topics[]` refreshed post-turn via `summarizeChat()`, which merges the latest exchange into the chat's existing summary/topics rather than rebuilding from scratch. `summarizeChatById()` (a fresh, non-incremental rebuild) runs immediately when a chat is moved into a project, and backfills any project member chat that has no summary yet (fire-and-forget from `GET /api/projects/{projectId}`).

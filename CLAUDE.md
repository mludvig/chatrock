...
- **Passive enrichment** (`sendMessage.ts` post-turn block, `lib/enrichment.ts`): runs per turn when `memoryEnabled`. `enrichUserFacts(transcript, existing, chatId?)` (Sonnet, JSON response) always runs and returns an updated user memory list; `reconcile()` + `putUserMemory` persist user ADDs/UPDATEs/DELETEs. When the chat belongs to a project, `enrichProjectFacts(transcript, existing, chatId?)` (Sonnet, JSON response) additionally returns an updated project memory list + summary; `reconcile()` + `putProjectMemory` persist project ops, `updateChatSummary` when `summary` present. Title generation is a **separate, independent call** — `generateChatTitle(transcript, chatId?)` (Haiku, plain-text response, no JSON parsing) — gated by `needTitle` (first normal send while `chat.title === 'New Chat'`); on success: `updateChatTitle` + `titleUpdated` frame. Splitting title out of the memory JSON means a malformed/truncated memory response can never silently suppress the title (or vice versa) — each of the three calls has its own try/catch and logs on failure (`enrich_user_facts_error`/`_parse_error`, `enrich_project_facts_error`/`_parse_error`, `generate_title_error`/`_empty_response`) rather than swallowing silently. Errors in the DB-write phase (outside these three calls) emit one `warning` WS frame and log `enrich_turn_error`.
...
- **Chat summaries**: `chat.summary` is refreshed post-turn via `enrichProjectFacts` (the same Sonnet call that extracts project facts). `enrichChatForProject()` runs immediately when a chat is moved into a project.
